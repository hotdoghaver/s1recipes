...
function init() {
    ...
    document.getElementById('sortByProfit')?.addEventListener('change', (e) => {
...
function renderStrainsList(strainArray = strains) {
    ...
    strainArray.forEach((strain, index) => {
        ...
        if (strain.id === currentStrainId) strainDiv.classList.add('active');
        if (strain.profit < 70) {
      ...
      `;                      
        ...
        strainDiv.addEventListener('click', () => {
            const strainId = strain.id;
            const realIndex = strains.findIndex(s => s.id === strainId);
            if (realIndex !== -1) {
                selectStrain(realIndex);
            }
        });
...
function selectStrain(index) {
    const selectedStrain = strains[index];
    currentStrainIndex = index; 
    currentStrainId = selectedStrain.id;
    renderSortedStrainsList();
    renderStrainDetails(selectedStrain);
}                             
...
passwordForm.addEventListener('submit', (e) => {
    ...
    if (inputPassword !== DELETE_PASSWORD) {
        showStatusMessage("Incorrect password. Try again.", "error");
        return;
    }

    // Close the password modal and show the final confirmation
    closeModal();
